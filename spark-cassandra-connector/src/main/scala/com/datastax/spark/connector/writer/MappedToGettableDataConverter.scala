...
import com.datastax.spark.connector.{TupleValue, UDTValue, ColumnRef}
...
private[connector] object MappedToGettableDataConverter {
  ...
  def apply[T : TypeTag : ColumnMapper]
    ...
    new TypeConverter[struct.ValueRepr] {
      ...
      /** For these types we don't want to use column mappers */
      private val dynamicStructRepresentations = Set(
        TupleValue.Symbol.fullName,
        UDTValue.Symbol.fullName,
        "com.datastax.spark.connector.japi.TupleValue",
        "com.datastax.spark.connector.japi.UDTValue")

      /** Returns true for structs that can store dynamic set of fields which we don't
        * know Scala types at the moment of searching for the converters. We must not
        * attempt to obtain ColumnMappers for these types. */
      private def isDynamicStructRepresentation(sym: Symbol): Boolean =
        dynamicStructRepresentations.contains(sym.fullName)
      ...
      private def converter[U : TypeTag](columnType: ColumnType[_]): TypeConverter[_ <: AnyRef] = {
        TypeTag.synchronized {
          ...
          (columnType, scalaType) match {
            ...
            // UDTValue, TupleValue:
            case (t: StructDef, TypeRef(_, symbol, _)) if isDynamicStructRepresentation(symbol) =>
